...
bendSolver    = TotalBendingSolver(memory, N, 25*23)
# pbd
...
def init():
    ...
    for i in range(len(cloth1.points)):
        memory.update(i, cloth1.points[i] + vec3(z=(ti.sin(i*np.pi/25)) * .05 ), 1.)
    for i in range(25):
        memory.invM[25*i+24] = 0.
    ...
    for i in range(25):
        for j in range(23):
            s = i*25
            bendSolver.update(i*23+j, s+j,s+j+1,s+j+2)
    bendSolver.init()  
...
def render(gui, pos2):
    ...
    gui.text(content=f'Stiffness={bendSolver.K[None]}',pos=(0,0.95), color=0xffffff)
...
###
###
gui = ti.GUI('Bend', background_color=0x112F41)
...
while gui.running:
    ...
    eventHandler.regularReact(camera=camera, stiffness_field=bendSolver.K, iteration_field=pbd.iters, mass_field=memory.invM,
                            pos2d=pos2,
